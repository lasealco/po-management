...
const SNIPPETS: AssistRetrievedSnippet[] = [
  {
    ...
    summary:
      "Carrier / TMS milestones can be pushed with POST /api/integrations/control-tower/inbound (Bearer or x-ct-inbound secret). Use payloadFormat canonical, generic_carrier_v1, carrier_webhook_v1 (data[] batch, default cap 50, env CONTROL_TOWER_INBOUND_CARRIER_WEBHOOK_MAX_ROWS up to 200), tms_event_v1, or visibility_flat_v1; optional idempotencyKey for safe replays.",
    detail:
      "Inbound integration: verify CONTROL_TOWER_INBOUND_WEBHOOK_SECRET. JSON body supports idempotencyKey (replays return the first response from CtAuditLog), payloadFormat canonical | generic_carrier_v1 | carrier_webhook_v1 | tms_event_v1 | visibility_flat_v1, and optional milestone upsert as CtTrackingMilestone with sourceType INTEGRATION. Canonical uses shipmentId + milestone object; generic_carrier_v1 uses carrierPayload; carrier_webhook_v1 uses non-empty data[] (each row like generic, batch cap default 50 / max 200 via CONTROL_TOWER_INBOUND_CARRIER_WEBHOOK_MAX_ROWS, per-row errors in response rows[]; JSON responses echo maxBatchRows; idempotencyKey suffix :index per row); tms_event_v1 uses tmsPayload; visibility_flat_v1 uses visibilityPayload with flat shipment id, event/status code, and timestamp fields.",
  },
  {
