...
export function ReportingCockpitClient({
  ...
  return (
    <section className="mb-8 space-y-4">
      ...
      <article className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-base font-semibold text-zinc-900">Suggested Next Actions</h3>
          <span className="text-xs text-zinc-500">Auto-prioritized playbook</span>
        </div>
        {snapshot.recommendedActions.length === 0 ? (
          <p className="rounded border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-900">
            No urgent actions detected right now. Great operational posture.
          </p>
        ) : (
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span
                    className={`rounded px-1.5 py-0.5 text-[11px] font-semibold ${
                      a.priority === "P1" ? "bg-rose-100 text-rose-900" : "bg-amber-100 text-amber-900"
                    }`}
                  >
                    {a.priority}
                  </span>
                </div>
                <p className="mt-1 text-xs text-zinc-600">{a.reason}</p>
                <Link href={a.href} className="mt-2 inline-block text-xs font-medium text-sky-800 hover:underline">
