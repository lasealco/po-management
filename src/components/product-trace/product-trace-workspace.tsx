...
export async function ProductTraceWorkspace({
  searchParams,
}: {
  ...
  if (!viewerHas(access.grantSet, "org.orders", "view")) {
    return (
      <div className="min-h-screen bg-zinc-50">
        <AccessDenied
          title="Product trace"
          message="You need org.orders → view (same gate as sales orders in this tenant)."
        />
      </div>
    );
  }
    ...
    <main className="mx-auto w-full max-w-5xl px-6 py-10">
      <header className="mb-8">
        ...
        <p className="mt-1 max-w-2xl text-sm text-zinc-600">
          Customer-safe view: suppliers, simulated in-transit positions along booking lanes, and warehouse stock (when
