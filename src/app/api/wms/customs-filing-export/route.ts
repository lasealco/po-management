...
import { buildCustomsFilingExportV1 } from "@/lib/wms/customs-filing-bf68";
...
export async function GET(request: Request) {
  ...
  if (order.status !== "PACKED" && order.status !== "SHIPPED") {
    return toApiErrorResponse({
      error: "Customs filing export is available when the outbound is PACKED or SHIPPED.",
      code: "BAD_REQUEST",
      status: 400,
    });
  }
  ...
  const snapshot = buildCustomsFilingExportV1(order, tenantRow, generatedAt);
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${safeName}-customs-filing-bf68.json"`,
