 ...
 * - `payloadFormat` — `canonical` (default), `generic_carrier_v1`, **`sea_port_track_v1`** (example carrier mapper: `seaPortEvent` → internal codes), **`carrier_webhook_v1`**, **`tms_event_v1`**, or **`visibility_flat_v1`**.
 ...
 * - `seaPortEvent` (`sea_port_track_v1`) — illustrative **SeaPort**-style object: `consignmentCuid|shipmentId|shipment_id`, `activityType|activity_type`, `eventTimestamp|eventUtc` (or snake_case), optional `freeText|narrative`, `eventRef|portalRef`. Mapped in `inbound-carrier-mappers.ts` to `generic_carrier_v1` milestone codes.
 */
