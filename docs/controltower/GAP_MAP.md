...
| 2026-04-23 | **Report PDF** (`report-pdf.ts` + `report-pdf-load-logo.ts`): **typography** tokens (title 14pt, consistent header/footer sizes); **optional** top-right **PNG/JPEG** (`reportLogoPngOrJpegBytes` + `reportLogoMime`); **scheduled** emails load logo once per cron run from **`CONTROL_TOWER_REPORT_PDF_LOGO_URL`** (https, size-capped). Builder **Download PDF** unchanged unless a future client fetch path is added. **GAP** R3 + changelog. |
