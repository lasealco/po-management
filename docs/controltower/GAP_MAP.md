...
| Area | Repo reality | Notes |
|------|--------------|--------|
| Inbound webhooks (carrier, forwarder) | 🟡 Secret + audit + idempotent milestone upsert + **`generic_carrier_v1`** + **`sea_port_track_v1`** (`seaPortEvent` → mappers) + **`simple_carrier_event_v1`** (flat top-level → generic) + **`carrier_webhook_v1`** (`data[]` batch, default cap **50**, env **`CONTROL_TOWER_INBOUND_CARRIER_WEBHOOK_MAX_ROWS`** up to **200**; responses + batch audit include **`maxBatchRows`**) + **`tms_event_v1`** + **`visibility_flat_v1`** | `POST /api/integrations/control-tower/inbound`, `inbound-webhook.ts`; Vitest **`inbound-webhook.test.ts`** + **`inbound-carrier-mappers.test.ts`**; add more carrier mappers alongside **`inbound-carrier-mappers.ts`** as needed |
...
| `POST …/integrations/control-tower/inbound` | Inbound webhook: secret + audit + **`idempotencyKey`**, **`generic_carrier_v1`** / **`sea_port_track_v1`** ( **`seaPortEvent`** → `inbound-carrier-mappers.ts` ) / **`simple_carrier_event_v1`** (flat → generic) / **`carrier_webhook_v1`** (batch cap **50** default, env up to **200**; **`maxBatchRows`** on JSON + audit) / **`tms_event_v1`** / **`visibility_flat_v1`**, **`CtTrackingMilestone`** upsert (`INTEGRATION`) |
...
3. ~~**Integration stub**~~ — 🟡 `POST /api/integrations/control-tower/inbound` + audit; **idempotent replays** (`idempotencyKey` + `INBOUND_WEBHOOK_EVENT` audit), **`generic_carrier_v1` / `sea_port_track_v1` / `simple_carrier_event_v1` (flat) / `carrier_webhook_v1` / `tms_event_v1` / `visibility_flat_v1`** + canonical milestone mapping, **`CtTrackingMilestone`** upsert — **Phase 1E:** mappers in `inbound-carrier-mappers.ts` + `seaPortEvent` example (`carrier_webhook_v1` batch cap: env up to 200).
4. **Assist / chatbot** (vs **`control_tower_search_and_chatbot_spec_*.pdf`** — **Assist / chatbot — gap vs PDF** subsection under **R3** + [issue #6](https://github.com/lasealco/po-management/issues/6))
   - **Done:** rule-based routing + optional LLM merge; **keyword** doc retrieval (`assist-retrieval.ts`) + optional **embedding hybrid** (`assist-retrieval-embed.ts`, `CONTROL_TOWER_ASSIST_EMBEDDINGS=1`, keyword fallback on failure or low confidence) feeding hints + `retrievedDocSnippets`; saved **CT report** names + saved **workbench filter** names in assist payload; broad snippet set (search, schedules, cron routes, digest, parties, legs/cargo, etc.) — see **R3 Assist** row above; **gap checklist** for the next implementer lives in the **R3** subsection (issue **#6**). **2026-04-25:** read-only **`postActionToolCatalog`** on **`POST …/assist`** (`assist-tool-catalog.ts`, sample **action** roster + router count) + **Search** collapsible reference + snippet **`control-tower-post-actions`**; **2026-04-23:** allowlisted **audited** **`POST …/assist/execute-post-action`** for **`acknowledge_ct_alert`** + **`create_ct_note`** (`confirmed: true` + `assistContext`), **`AssistPostAction`** audit, Search UI; **not** model-initiated tool loops.
   - **Partial:** **per-request** assistant, not threaded chat sessions; no dedicated **vector DB** / chunking pipeline — embeddings are over the **static** assist corpus only; no LLM-proposed then operator-confirmed tool loop from the model path yet; no autonomous agent loops.
   - **Next smallest slice:** expand allowlist + **optional** “suggested action” JSON from LLM with the same **confirm** gate **or** richer embedding ops (re-rank, external corpus) — before chasing full PDF parity.
...
6. **Workbench**
   - **Done:** filters (incl. **open exception code** + **open alert type**), **saved views** (`CtSavedFilter`), **column visibility** (browser `localStorage` + optional **`columnVisibility` on saved view**), CSV export aligned to visible columns + **`# …` cap line** when truncated, **list cap** surfaced in UI, **deep links** from hub + executive + reports → `controlTowerWorkbenchPath` (status, overdue ETA, drills), **row multi-select** + **bulk acknowledge open alerts** (`bulk_acknowledge_ct_alerts`) + **bulk set shipment ops assignee** (`bulk_update_shipment_ops_assignee`, **`GET …/workbench-assignees`**) on `/control-tower/workbench`. **2026-04-23:** **bulk exception owner** — `bulk_assign_ct_exception_owner` (selected rows with open/in-progress exceptions; `ownerUserId` or clear).
   - **Partial / gap:** no **server-stored default column visibility** per actor outside **`columnVisibility` inside saved-view JSON**.
   - **Next smallest slice:** **server-stored default column visibility** per actor.
...
- ~~**`[tower] Assist: audited tool calls for Control Tower POST actions`**~~ — **Landed (2026-04-23):** `assist-post-action-allowlist.ts` + `POST /api/control-tower/assist/execute-post-action` (`acknowledge_ct_alert`, `create_ct_note`); `assistExecutablePostActionToolCatalog` on assist; Search “Confirm and run action”. **Next:** more allowlisted actions, optional LLM-suggested payload + same confirm path.
...
- ~~**`[tower] Inbound: carrier-specific mapper example`**~~ — **Landed (2026-04-25):** `sea_port_track_v1` + `inbound-carrier-mappers.ts` + tests. **2026-04-23:** `simple_carrier_event_v1` (flat `shipmentId` + `eventCode` + `eventTime`). **Next:** next real carrier `payloadFormat` or shared mapper util.
...
| Date | Change |
|------|--------|
| 2026-04-23 | **Phase 3:** workbench **`bulk_assign_ct_exception_owner`**; inbound **`simple_carrier_event_v1`**; GAP **#6** / **#3** / route table / post-action inventory. |
