...
- **Notifications:** `GET /api/srm/notifications?unread=1` filters to unread; **POST** `{ markAllRead: true }` marks all for the current user; notifications page: **Unread only** toggle, **Mark all as read** (primary CTA). List JSON includes **`actorUserId`**, **`actorName`**, **`supplierName`**, and **`supplierCode`** when linked; the page shows **From …** and **Supplier: … (code)** on the meta line, aligned with opt-in email and webhook. The main **SRM** surfaces (list, supplier **360**, **analytics**, **new partner** when the user can view suppliers) use the same **Notifications** link with the **unread** count badge.
- **Optional email mirror (G follow-up):** set `SRM_OPERATOR_EMAIL_NOTIFICATIONS=1` plus existing Resend vars (`RESEND_API_KEY`, and `SRM_EMAIL_FROM` or `CONTROL_TOWER_REPORTS_EMAIL_FROM`) to send a plain-text email to the notification recipient when an in-app row is created (e.g. onboarding assignee). The body includes a **From:** line with the assigner’s **display name** when the event has an actor, and a **Supplier:** line (name, plus code in parentheses when available) when the event is linked to a supplier, matching webhook context. Off by default; in-app rows remain the source of truth.
- **Optional outbound webhook (G follow-up):** set `SRM_OPERATOR_WEBHOOK_URL` to a valid `http`/`https` URL to `POST` JSON (`specVersion: 1`, `event: srm.operator_notification.created`, `notification: { id, tenantId, …, supplierId, supplierName, supplierCode, …, actorUserId, actorName, … }`). `actorName` is the assigner’s display name when resolvable, else `null`. `supplierName` / `supplierCode` are resolved when `supplierId` is set, else `null`. Optional `SRM_OPERATOR_WEBHOOK_SECRET` is sent as `X-SRM-Webhook-Secret`. Off by default; in-app rows remain the source of truth.
