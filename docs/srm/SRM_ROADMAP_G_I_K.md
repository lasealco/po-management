...
- **Notifications:** `GET /api/srm/notifications?unread=1` filters to unread; **POST** `{ markAllRead: true }` marks all for the current user; notifications page: **Unread only** toggle, **Mark all as read** (primary CTA). List JSON includes **`actorUserId`**, **`actorName`**, **`supplierName`**, and **`supplierCode`** when linked; the page shows **From …** and **Supplier: … (code)** on the meta line, aligned with opt-in email and webhook. The **SRM list** and **SRM supplier 360** (`/srm/[id]`) headers both link to `/srm/notifications` with the same **unread** count badge.
