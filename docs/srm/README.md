...
- **Onboarding intake:** `POST /api/suppliers` persists optional **`internalNotes`**, seeds the **default checklist** immediately, and sets **`qualificationStatus`** to **in progress** for partners that start **pending approval**. `/srm/new` collects intake notes and redirects to **`/srm/[id]?tab=onboarding`** after create. The checklist block is anchored at **`#supplier-onboarding-section`** with **Jump to next task** scrolling to the highlighted next row.
...
- **URLs:** Supplier 360 under `/srm/[id]` keeps the active workspace in **`?tab=`** (e.g. `overview`, `onboarding`, `compliance`, `contacts`, `documents`) so refresh and shared links reopen the same tab. **`?registerCategory=`** (`insurance`, `license`, `certificate`, `compliance_other`, `commercial_other`) opens **Documents** with that category pre-selected in the register flow; the param is removed after apply. On legacy **`/suppliers/[id]`**, the same param is supported (Compliance **Register** sets it briefly; consumption scrolls to the documents block). The SRM directory (and legacy **`/suppliers`**) support **`?kind=`**, **`?q=`** (name/code), **`?approval=`** (`all` \| `pending` \| `approved` \| `rejected`), **`?active=`** (`all` \| `active` \| `inactive`), and **`?sort=`** (`name` \| `code` \| `updated`).
- **Compliance / performance / risk:** REST under `/api/suppliers/[id]/compliance-reviews`, `…/performance-scorecards`, `…/risk-records` (POST create; PATCH row; **DELETE** by id on nested review / scorecard / risk for mistaken or obsolete rows). Data loads with the supplier snapshot for `/srm/[id]` and legacy detail. The **Compliance** tab **document control** strip uses **tiered expiry** (critical ≤14 days, soon ≤30 days, then expired), a **readiness score** (0–100 heuristic), **missing controlled document types** (no insurance/license/certificate row on file) with **Register {category}** actions that jump to **Documents** with the category pre-selected (SRM shell: same **`?registerCategory=`** query as manual deep links — param is stripped after apply), row-level findings with detail lines, plus periodic review **next due** hints. A **Compliance reviews** control jumps to the periodic reviews section (SRM: stays on Compliance tab and scrolls). Workspace tabs show **issue counts** on **Compliance** and **Documents** when attention is needed. **Performance** / **Risk** tabs show small **R1-only** counts on the SRM nav: scorecard rows missing both on-time % and quality (not analytics), and risks not in **closed** status (open pipeline). Lists also support blur-save **notes** (scorecards) and **details** (risks) on existing rows — same PATCH APIs, no rollups. **Compliance reviews** support blur-save **summary** and **next review due** on existing rows; reviews with **Action required** or **Failed** show a short **suggested follow-up** strip (Documents / Risk shortcuts on `/srm/[id]`, legacy scrolls) — not a separate action-plan entity in R1. **Add review** pre-fills **next review due** with a ~**180-day** suggested date (clear if unknown).
- **Documents:** `POST/PATCH/DELETE` `/api/suppliers/[id]/documents` — metadata + optional `http(s)` reference URL + optional **`expiresAt`** + optional soft **`archivedAt`** via PATCH body `{ "archived": true|false }` (archived rows are omitted from Compliance readiness counts). The **Documents** tab includes **All / Active / Archived / Needs attention** filters (row-level expiry + missing expiry on controlled rows); if **Needs attention** is empty but required categories are still absent, an inline hint points back to **Compliance** for supplier-level gaps. Row badges reflect **critical** vs **soon** expiry tiers. On **active** rows, editors can blur-save **title**, **reference URL**, **document date**, **notes**, **expiry**, and change **category** (same PATCH API). **Copy link** copies the saved reference URL; **Remove** confirms archive-vs-delete guidance. Not automated workflow, tender, tariff, or sourcing events.
