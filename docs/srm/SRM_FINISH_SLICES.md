...
| **K** | **Enterprise polish** | **Field-level** permission matrix, **pixel/wireframe** pack parity, **workflow/rules** engine depth, multi-tenant cross-org (if in scope) — *pick as separate issues*. **Started in repo:** `canViewSupplierSensitiveFields` on **`resolveSrmPermissions`** (`org.suppliers` → **edit** or **approve**); **`internalNotes` redacted** on **`GET /api/suppliers/[id]`** and in supplier 360 when view-only; inline copy on profile. **Still future:** full matrix, wireframe parity, rules engine, cross-tenant. |
