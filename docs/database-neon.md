...
## SRM (finish program) ó demo partners on `demo-company`
...
- **Idempotent:** upserts five suppliers with codes **`DEMO-SRM-001`Ö`DEMO-SRM-005`**, recreates **compliance** file metadata rows (file names contain `srm-demo-seed`), and refreshes default **onboarding** tasks with mixed completion and assignees.
...
- **Neon / Vercel:** same as other add-on seeds ó not run in `vercel-build` by default; run manually against the envís **`DATABASE_URL`** when you need the slice-29 dataset on a fresh database.
