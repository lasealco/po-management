...
2. **In progress, module by module:** same org/division read rules on **Control Tower** (workbench, hub, ops/reports summaries, digest, `runControlTowerReport`, product trace) by scoping on the shipment’s **linked purchase order** (requester + line divisions). **CRM** list/detail/convert and **WMS** CRM account pickers/links apply the same org/division idea to the CRM record **owner** (`getCrmAccessScope` in `src/lib/crm-scope.ts`). **WMS** workbench and billing (`GET /api/wms`, `GET /api/wms/billing`) use `loadWmsViewReadScope` in `src/lib/wms/wms-read-scope.ts`: **tasks / waves** along linked **purchase order** and **shipment (Control Tower)**, with internal tasks by **createdBy** when org rules apply; **outbounds** and **billing events** by **CRM** account + creator; **billing invoice runs** (list, CSV download, `post_invoice_run`) are limited to runs **you created** (org-scoped `User`) or that contain at least one **visible billing event**; **inventory / movement ledger / replen rules** by **product division**; **inbound shipment** lists match **CT shipment** rules.
...
- [ ] **(a) Scoped permissions / policy layer:** extend RBAC or add a policy engine; audit remaining `tenantId`-only queries (CRM, WMS, other CT surfaces, etc.). → *Phase 8 (in progress)*
...
- [ ] **(b) Portals:** customer users, invitation flow, least-privilege roles; extend visibility rules beyond Control Tower. → *Phase 6 (in progress)*
- [ ] **Reporting:** remaining hubs respect **active scope** (PO-based reports and CT **shipment** reports use org scope; widen to other datasets as modeled). → *Phase 7 (in progress)*
...
| **6** | **Portals** (*in progress*) | **PO / product-trace / CT paths** that use `getPurchaseOrderScopeWhere` now **AND** a **customer CRM match** when `User.customerCrmAccountId` is set (with org/division when applicable). Seeded **Customer portal** role includes **`org.orders` → view** (read-only, scoped by that filter). **Consolidation** and **SCT/SCRI** nav entries hidden for customer-scoped users. **Invites** / full least-privilege matrix still open. | **(b) Portals** |
| **7** | **Reporting** (*in progress*) | **Executive cockpit** (`buildExecutiveSummary`) uses `loadWmsViewReadScope`, `purchaseOrderWhereWithViewerScope`, and CRM owner scope. **Reporting hub cockpit** (`buildReportingCockpitSnapshot` when `actorUserId` is set) applies the same WMS/CT/PO/CRM patterns. Registry reports: **shipment status summary** and **load plans by status** use `controlTowerShipmentAccessWhere`; **supplier performance** scopes parent POs per viewer. Order-based reports already used PO scope. | **Reporting** |
| **8** | **Policy + read-model audit** (*in progress*) | **Canonical read-scope module** `src/lib/viewer-scopes.ts` (re-exports + `loadViewerReadScopeBundle`); **inventory + checklist** in `docs/engineering/READ_SCOPE_INVENTORY.md`; **advisory script** `npm run read-scope:audit-hints` (heuristic: API routes with Prisma + `tenantId` and no common scope markers). A **dedicated policy engine** (rule DSL, effect matrix) is still a **product** track. | **(a) Scoped permissions / policy layer** |
...
- Prisma: `Tenant`, `User` (`tenantId`, `primaryOrgUnitId`, `customerCrmAccountId`), `OrgUnit`, `UserProductDivision`, `CrmAccount` (`parentAccountId`). **Phase 5:** `PurchaseOrder.customerCrmAccountId` (optional 3PL/BCO customer) aligns with `Shipment.customerCrmAccountId` / `SalesOrder.customerCrmAccountId`; supplier ASN shipment create and CT logistics shipment create copy from the PO when present; WMS billing CRM resolution includes `SHIPMENT_ITEM` movements via the linked shipment. **Phase 6:** `getPurchaseOrderScopeWhere` (in `src/lib/org-scope.ts`) applies that customer id for users with `User.customerCrmAccountId`; `actorIsCustomerCrmScoped` in `src/lib/authz.ts` for internal-only pages/nav. **Phase 7:** executive summary and reporting cockpit (`src/lib/executive/summary.ts`, `src/lib/reporting/cockpit-data.ts`) plus report definitions in `src/lib/reports/definitions/` align with WMS/PO/CT/CRM read scope when the viewer is known. **Phase 8:** `src/lib/viewer-scopes.ts` (re-exports + `loadViewerReadScopeBundle`); `docs/engineering/READ_SCOPE_INVENTORY.md`; `npm run read-scope:audit-hints` for advisory triage of API routes.
...
- **2026-04-25:** **Phase 8 (started):** `src/lib/viewer-scopes.ts` centralizes read-scope re-exports and `loadViewerReadScopeBundle`; `docs/engineering/READ_SCOPE_INVENTORY.md` documents helpers, rollout status, and a PR checklist; `scripts/read-scope-audit-hints.mjs` + `npm run read-scope:audit-hints` (heuristic, advisory) to surface API routes for manual review. A formal **policy engine** (rule DSL) remains a product decision.
- **2026-04-25:** **Phase 7 (started):** Executive summary and **reporting cockpit** snapshot apply org/division/customer/supplier portal scope (WMS inventory by product division, CT shipment rules, CRM owner scope, scoped WMS billing aggregates). Reports **shipment_status_summary**, **load_plans_by_status**, and **supplier_performance** respect viewer scope; `fetchSupplierPerformanceSummary` takes actor + supplier-portal flag.
- **2026-04-25:** **Phase 6 (started):** `getPurchaseOrderScopeWhere` restricts users with `User.customerCrmAccountId` to purchase orders with the same `customerCrmAccountId` (in addition to org/division rules). `actorIsCustomerCrmScoped` in authz; nav hides consolidation / supply-chain twin / SCRI for those users; consolidation page defends; demo **Customer portal** role gets **`org.orders` → view** so orders board and product trace work under that scope.
