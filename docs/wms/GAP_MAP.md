...
| Zone / aisle / bay… | 🟡 `WarehouseZone` + `WarehouseBin` | **WE-10** + **BF-04** [`WMS_ZONE_TOPOLOGY_ADR.md`](./WMS_ZONE_TOPOLOGY_ADR.md), [`WMS_ZONE_PARENT_BF04.md`](./WMS_ZONE_PARENT_BF04.md): functional zones per warehouse + optional **`parentZoneId`** (DAG, `set_zone_parent`) + bins with **`rackCode` / `aisle` / `bay` / `level` / `positionIndex`** + Setup **rack front map**; **first-class aisle entities** & mm/3D geometry deferred |
...
| SKU / UOM / lot rules | 🟡 **`Product`** + **`InventoryBalance.lotCode`** + **`WmsLotBatch`** (BF-02) | Shared catalog SKU; batch buckets per bin/SKU ([`WMS_LOT_SERIAL_DECISION.md`](./WMS_LOT_SERIAL_DECISION.md)). Ops/regulatory metadata (expiry, origin, notes) via [`WMS_LOT_BATCH_BF02.md`](./WMS_LOT_BATCH_BF02.md) + `set_wms_lot_batch`; **per-unit serial genealogy** still backlog |
...
| **Inbound ASN** | 🟡 **Orders + `Shipment` / `ShipmentItem`** | `Shipment.asnReference` + `expectedReceiveAt`; **`wmsReceiveStatus`** (+ note/timestamps/user) + [`set_wms_receiving_status`](./WMS_RECEIVING_STATE_MACHINE_SPEC.md) / [`WMS_RECEIVING_OPTION_A.md`](./WMS_RECEIVING_OPTION_A.md); WMS inbound table + `set_shipment_inbound_fields`; **BF-01** line variance ([`WMS_RECEIVING_LINE_VARIANCE_BF01.md`](./WMS_RECEIVING_LINE_VARIANCE_BF01.md)) — `ShipmentItem.quantityReceived` + `wmsVarianceDisposition` / `wmsVarianceNote`, `set_shipment_item_receive_line`; separate **`WmsReceipt`** header table / multi-event dock receipts still backlog |
...
| Allocation | 🟡 **`Warehouse.pickAllocationStrategy`** (`MAX_AVAILABLE_FIRST` \| `FIFO_BY_BIN_CODE` \| **`FEFO_BY_LOT_EXPIRY`** \| `MANUAL_ONLY`) + `orderPickSlotsForWave` tests ([`WMS_ALLOCATION_STRATEGIES.md`](./WMS_ALLOCATION_STRATEGIES.md)) | **BF-03:** FEFO waves use **`WmsLotBatch`** expiry ordering; other strategies remain fungible-only for waves. No carton solver / multi-wave optimizer |
...
| Appointment scheduling | 🟡 `WmsDockAppointment` + `GET/POST /api/wms` | Tenant-scoped dock code + window + inbound shipment or outbound order link; **overlap blocked** for `SCHEDULED`; cancel action; **BF-05** carrier/trailer + yard timestamps ([`WMS_DOCK_YARD_BF05.md`](./WMS_DOCK_YARD_BF05.md)); WMS Operations UI — not full TMS/carrier EDI ([`WMS_DOCK_APPOINTMENTS.md`](./WMS_DOCK_APPOINTMENTS.md)) |
...
`create_zone`, `set_zone_parent`, `create_bin`, `update_bin_profile`, `set_replenishment_rule`, `create_replenishment_tasks`, `create_outbound_order` (optional `crmAccountId`, optional `asnReference` + `requestedShipDate`), `set_outbound_crm_account`, `set_outbound_order_asn_fields`, `release_outbound_order`, `create_putaway_task`, `complete_putaway_task`, `create_pick_task`, `create_pick_wave`, `set_warehouse_pick_allocation_strategy`, `release_wave`, `complete_wave`, `complete_pick_task`, `mark_outbound_packed`, `mark_outbound_shipped`, `set_shipment_inbound_fields`, `set_wms_receiving_status`, `set_shipment_item_receive_line`, `set_wms_lot_batch`, `create_dock_appointment`, `cancel_dock_appointment`, `set_dock_appointment_transport`, `record_dock_appointment_yard_milestone`, `record_shipment_milestone`, `set_balance_hold`, `clear_balance_hold`, `complete_replenish_task`, `create_cycle_count_task`, `complete_cycle_count_task`, `create_work_order`, `create_value_add_task`, `complete_value_add_task`.
...
_Next optional increments:_ ~~saved ledger views~~ (**landed** 2026-04-23: `WmsSavedLedgerView` + `/api/wms/saved-ledger-views`); ~~**outbound ASN** parity~~ (**landed** 2026-04-25: `OutboundOrder.asnReference` + `requestedShipDate` in payload + `set_outbound_order_asn_fields`); **2.3 deeper receiving states** — **Option A landed** 2026-04-29 (`WmsReceiveStatus`, `set_wms_receiving_status`, audit); **BF-01** line variance landed (`set_shipment_item_receive_line`, [`WMS_RECEIVING_LINE_VARIANCE_BF01.md`](./WMS_RECEIVING_LINE_VARIANCE_BF01.md)); Option B **`WmsReceipt`** header / multi-event receipts still backlog — [`WMS_RECEIVING_STATE_MACHINE_SPEC.md`](./WMS_RECEIVING_STATE_MACHINE_SPEC.md). **Dock appointments** — **WE-02 + BF-05** (`WmsDockAppointment`, yard milestones — [`WMS_DOCK_YARD_BF05.md`](./WMS_DOCK_YARD_BF05.md), [`WMS_DOCK_APPOINTMENTS.md`](./WMS_DOCK_APPOINTMENTS.md)); full TMS/carrier EDI still out of scope. **Allocation profiles** — **WE-03 landed** 2026-04-29 ([`WMS_ALLOCATION_STRATEGIES.md`](./WMS_ALLOCATION_STRATEGIES.md)); FEFO/solver engines still out of scope.

_Last updated: 2026-04-29 — **BF-05** dock yard carrier + milestones (`WMS_DOCK_YARD_BF05.md`, `set_dock_appointment_transport`, `record_dock_appointment_yard_milestone`); **BF-04** zone parent hierarchy (`WMS_ZONE_PARENT_BF04.md`, `set_zone_parent`, `WarehouseZone.parentZoneId`); **BF-03** `FEFO_BY_LOT_EXPIRY` wave allocation (`WMS_ALLOCATION_STRATEGIES.md`); **BF-02** lot batch master (`WMS_LOT_BATCH_BF02.md`, `set_wms_lot_batch`, `WmsLotBatch`); **`BF_CAPSULE_ROADMAP`** (`BF-02`–`BF-11` order); **BF-01** receiving line variance (`WMS_RECEIVING_LINE_VARIANCE_BF01.md`, `set_shipment_item_receive_line`); tranche paragraphs aligned; **Blueprint finish backlog** ([`BLUEPRINT_FINISH_BACKLOG.md`](./BLUEPRINT_FINISH_BACKLOG.md)); **WE-12 enterprise exit** ([`WMS_ENTERPRISE_EXIT_WE12.md`](./WMS_ENTERPRISE_EXIT_WE12.md)); WE-11 CT map Phase 3.4 (`WMS_CT_MAP_PHASE34_WE11.md`); WE-10 zone topology ADR (`WMS_ZONE_TOPOLOGY_ADR.md`); WE-09 executive KPI row + `homeKpis` export; WE-08 RBAC/audit doc + API grant tests; WE-07 commercial handoff; WE-06 packing; WE-05 lots; WE-04 VAS; WE-03 allocation; WE-02 dock; Phase 2.3 receiving (Option A); 2026-04-26 tranche handoff._
