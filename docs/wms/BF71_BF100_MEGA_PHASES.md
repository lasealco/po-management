...
**Shipped (minimal):** Enum scope **`INBOUND_ASN_ADVISE_WRITE`** on **`WmsPartnerApiKeyScope`**; **`POST /api/wms/partner/v1/mutations/inbound-asn-advise`** with **`partnerV1Json`** **`schemaVersion: "bf98.v1"`**; delegates to **`upsertInboundAsnAdviseRow`**; **`CtAuditLog`** **`bf98_partner_inbound_asn_advise_upsert`** with surrogate **`actorUserId`** (**first active tenant user**) and **`partnerApiKeyId`** in payload; Setup Partner API checkbox + docs **[`WMS_PARTNER_API_BF45.md`](./WMS_PARTNER_API_BF45.md)** / **[`openapi-partner-v1.yaml`](./openapi-partner-v1.yaml)**.
...
**Shipped (minimal):** **`GET /api/wms/search-snapshot`** returns **`application/x-ndjson`** with **`schemaVersion: bf99.v1`** per line; opaque **`cursor`** + **`limit`** (default 200, max 500); segments **shipments → outbound orders → tasks** with **`loadWmsViewReadScope`** filtering; response headers **`X-Search-Snapshot-Next-Cursor`**, **`X-Search-Snapshot-Line-Count`**, **`X-Search-Snapshot-Schema`**; Setup UI links + **[`WMS_SEARCH_SNAPSHOT_BF99.md`](./WMS_SEARCH_SNAPSHOT_BF99.md)**.
