...
**Exit sketch:** **`POST /api/wms/inbound-asn-normalize`** (raw payload + `partnerId`) → normalized **`linesJson`**.
...
**Exit sketch:** **`GET /api/wms/pick-path-export?waveId=`** (`bf76.v1`).
...
**Exit sketch:** Tenant policy JSON + cron or on-complete evaluator.
