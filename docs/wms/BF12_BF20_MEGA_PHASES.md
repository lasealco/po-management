...
**Minimal slice shipped (repo):** Global **`org.wms.inventory.lot`** (view / edit) in **`GLOBAL_PERMISSION_CATALOG`**; **`wms-field-acl-matrix.json`** drives **`inventoryAclKindForAction`** (+ **`WMS_POST_ACTIONS_LOT_METADATA_SCOPED`** alias for lot-metadata actions — **`set_wms_lot_batch`**) in **`src/lib/wms/wms-inventory-field-acl.ts`**; **`gateWmsPostMutation`** splits inventory-tier POST actions so **lot-only** editors cannot post holds, cycle counts, etc. (**BF-48** adds **`org.wms.inventory.serial`** + **`serial_registry`** kind — serialization POST actions without full **`org.wms.inventory` → edit** — [`WMS_RBAC_BF48.md`](./WMS_RBAC_BF48.md)). Stock workspace (**`/wms/stock`**) disables qty-path controls unless **`org.wms.inventory` → edit** (or legacy **`org.wms` → edit**); serialization mutate shell follows **`inventorySerialEdit`**. Buyer/Superuser demo seeds include **`inventory.lot`** and **`inventory.serial`** alongside **`inventory`**. Vitest: **`wms-inventory-field-acl.test.ts`**, **`wms-field-acl-matrix.test.ts`**.
