...
- ASN **auto-close** rules tied to carrier ASN payloads (855/JSON), tolerance policies per SKU beyond **BF-31** dock %-tol guardrails, centralized **`GRN`** numbering service, promotion of **BF-32** **`WmsReceivingAccrualStaging`** rows into finance postings (beyond CSV export), receipt **versioning** beyond audit logs.
