...
| 8 | **BF-09** | R3 | VAS portal & BOM costing ([`WMS_VAS_WORK_ORDERS.md`](./WMS_VAS_WORK_ORDERS.md)) — **WMS + portal**; commercial assumptions. |
| 9 | **BF-10** | E | CPQ → outbound automation ([`WMS_COMMERCIAL_HANDOFF.md`](./WMS_COMMERCIAL_HANDOFF.md)) — **CRM (+ WMS API)** joint milestone. |
| 10 | **BF-11** | E | CT map merged layers ([`WMS_CT_MAP_PHASE34_WE11.md`](./WMS_CT_MAP_PHASE34_WE11.md)) — **CT + WMS** (+ CRM pins if in scope); [`GAP_MAP`](./GAP_MAP.md) Enterprise row currently **❌** for in-map floor/pins. |

**Parallelization:** **BF-07** / **BF-08** can often run in parallel with **BF-04** / **BF-05** if teams differ — dependencies above are **logical**, not strict calendar locks.
...
_Last updated: 2026-04-29 — **BF-06** scoped WMS RBAC tiers (`WMS_RBAC_BF06.md`, `org.wms.setup` / `operations` / `inventory`); **BF-05** dock yard milestones (`WMS_DOCK_YARD_BF05.md`); **BF-03** FEFO allocation (`FEFO_BY_LOT_EXPIRY`); **BF-02** `WmsLotBatch` landed (`WMS_LOT_BATCH_BF02.md`, **`BF_CAPSULE_ROADMAP` Done**); roadmap scaffold after **BF-01**._
