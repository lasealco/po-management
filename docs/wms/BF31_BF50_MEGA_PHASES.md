...
| **BF-44** | Outbound webhooks / event subscriptions | **Minimal landed** — subscriptions + HMAC `sha256=<hex>` + delivery rows + immediate POST + backoff fields ([`WMS_OUTBOUND_WEBHOOKS_BF44.md`](./WMS_OUTBOUND_WEBHOOKS_BF44.md)); background retry worker (**BF-45**) backlog | Audit + idempotency patterns (**BF-25**) |
...
**Shipped:** [`WMS_OUTBOUND_WEBHOOKS_BF44.md`](./WMS_OUTBOUND_WEBHOOKS_BF44.md) — `WmsOutboundWebhookSubscription` / `WmsOutboundWebhookDelivery`, POST actions (`create_wms_outbound_webhook_subscription_bf44`, …), `emitWmsOutboundWebhooks` + Vitest, Setup UI panel, hooks after `close_wms_receipt`, `mark_outbound_shipped`, billing dispute.

**Deferred:** Background retry worker, DLQ UI (**BF-45** program overlap).
