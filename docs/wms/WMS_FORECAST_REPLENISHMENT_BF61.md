...
| **Batch** | **`create_replenishment_tasks`** loads stubs for **current UTC week**; **`forecastGapQty = max(0, forecast − pickFaceEffective)`** (same pick-face / zone filter and soft-reservation math as shortage logic); tiered **priority boost** (5 / 15 / 30 / 50) applied in **`sortReplenishmentRulesForBatch`**. **`WmsTask.replenishmentPriority`** snapshots **`rule.priority + boost`**. |
| **GET** | **`GET /api/wms`** includes **`demandForecastStubs`** (current week, scoped) and **`forecastGapHints`** (per active rule: forecast, pick-face effective, gap, boost, effective sort priority). |
| **UI** | **`/wms`** — **Demand forecast stub (BF-61)** panel: hint grid + saved stubs + save form. |

## Out of scope

Statistical or ML forecasting engines; ERP demand interfaces.

_Last updated: 2026-04-29 — BF-61 minimal stub._
