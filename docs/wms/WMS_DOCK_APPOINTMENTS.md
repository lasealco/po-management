...
- **BF-74:** **Yard geofence / arrival** webhook **`POST /api/wms/yard-geofence-webhook`** — Bearer **`WMS_YARD_GEOFENCE_WEBHOOK_SECRET`**, optional HMAC (**`WMS_YARD_GEOFENCE_WEBHOOK_HMAC_SECRET`**, **`X-Yard-Geofence-Signature`**), mandatory **`externalEventId`** (**`WmsYardGeofenceWebhookReceipt`**). Same milestone semantics + BF-38 + BF-54 as **`record_dock_appointment_yard_milestone`**. See [`WMS_YARD_GEOFENCE_BF74.md`](./WMS_YARD_GEOFENCE_BF74.md).
...
| **`POST /api/wms/yard-geofence-webhook`** (**BF-74**) | **No session cookie.** Bearer **`WMS_YARD_GEOFENCE_WEBHOOK_SECRET`**; optional **`WMS_YARD_GEOFENCE_WEBHOOK_HMAC_SECRET`** + **`X-Yard-Geofence-Signature`**. Body: **`dockAppointmentId`**, **`yardMilestone`**, required **`externalEventId`** (≤128 chars), optional **`yardOccurredAt`**, **`tenantSlug`**, optional **`schemaVersion: bf74.v1`**. Audited **`yard_geofence_webhook_bf74`**; **`dock_detention_breach`** when BF-54 applies. See [`WMS_YARD_GEOFENCE_BF74.md`](./WMS_YARD_GEOFENCE_BF74.md). |
