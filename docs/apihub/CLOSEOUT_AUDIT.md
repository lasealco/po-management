...
| R7 | **Secrets / PII in logs and error payloads** | Medium (residual: some Postgres constraint text may still pass the heuristic) | **P4 shipped:** leakage conformance + `apiHubOperatorMessageFromCaughtError` / `apiHubStagingBatchCreateFailedMessage` on staging create/apply, ingestion downstream apply, mapping-analysis job failure persistence. |
