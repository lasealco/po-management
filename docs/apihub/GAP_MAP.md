...
| App route `/apihub` | ✅ `src/app/apihub/**` | Analysis jobs, staging list + apply + discard, templates, connectors, ingestion triage. **Workspace** ingestion table: **Error code** column for **`failed`** rows; expand: **Retry run** when **`org.apihub` → edit**. **Slice 2 (2026-04-22):** client fetch failures that return JSON include **Advanced** error body where applicable (ops, alerts, conflicts, run expand + timeline, connector audit, rules diff, staging apply). |
