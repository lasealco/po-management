...
|------|--------------|--------|
| Tenant scoping (repos) | ✅ [`TENANT_SCOPING.md`](./TENANT_SCOPING.md), `*.tenant-scope.test.ts` | Mapping template PATCH/DELETE use `updateMany`/`deleteMany` with `{ id, tenantId }`; staging **discard** + connector **lifecycle** field mutations use `updateMany` `{ id, tenantId }`; tenant-scope tests incl. **ingestion in-flight count** + **ops `groupBy`** (**2026-04-22**) |
