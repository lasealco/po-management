...
|------|--------------|--------|
| Tenant scoping (repos) | ✅ [`TENANT_SCOPING.md`](./TENANT_SCOPING.md), `*.tenant-scope.test.ts` | Mapping template PATCH/DELETE use `updateMany`/`deleteMany` with `{ id, tenantId }`; staging **discard** + connector **lifecycle** use `updateMany` `{ id, tenantId }`; tests incl. **ingestion in-flight** + **ops `groupBy`** + **`$queryRaw` bind** (conflicts + alerts) + **ingestion run timeline page** (**2026-04-22**) |
...
4. P4 — **shipped (2026-04-22):** centralized JSON body tiers (`request-budget.ts`); every POST/PATCH route uses `*WithBudget` helpers (including no-body POSTs that still bounded-read); route conformance tests (`apihub-routes-conformance.test.ts`); stable `apiHubError` / `apiHubValidationError` envelope tests (`api-error.test.ts`); **`safe-server-log`** + **leakage conformance** (`apihub-leakage-conformance.test.ts`). **R9 partial:** public **`GET /api/apihub/health`** body is pinned to `getApiHubHealthJson()` + key allowlist (`health-body.test.ts`, `health/route.test.ts`); ApiHub **cron** **`401`/`503`** errors use shared **`toApiErrorResponse`** shape (`error` + `code` only) — `route.test.ts` + smoke pack.
