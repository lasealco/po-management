...
| Background workers | 🟡 | Mapping jobs: `after()` on create **plus** cron `GET/POST /api/cron/apihub-mapping-analysis-jobs` (Bearer `CRON_SECRET`, `vercel.json` every **10m** UTC on Pro) drains `queued` via shared `processApiHubMappingAnalysisJob` (R2 **2026-04-22**). No Redis queue; **stuck `processing`** reclaim not implemented. |
...
| R2 Workers | 🟡 **ApiHub mapping analysis:** Vercel cron + `CRON_SECRET` worker route (no separate queue). **Full ETL** / Redis still out of scope. |
