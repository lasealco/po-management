...
|------|--------------|--------|
| Tenant scoping (repos) | ✅ [`TENANT_SCOPING.md`](./TENANT_SCOPING.md), `*.tenant-scope.test.ts` | Mapping template PATCH/DELETE use `updateMany`/`deleteMany` with `{ id, tenantId }`; staging **discard** + connector **lifecycle** use `updateMany` `{ id, tenantId }`; tests incl. **ingestion in-flight** + **ops `groupBy`** + **`$queryRaw` tenant bind** (apply conflicts + alerts summary) (**2026-04-22**) |
