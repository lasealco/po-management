...
- [ ] **`smoke:apihub` exits 0** — checks `GET /api/apihub/health` (JSON), `GET /apihub` (guided import shell), and `GET /apihub/workspace` (full workspace when signed in with **`org.apihub`**, otherwise the **API hub** access gate HTML).
