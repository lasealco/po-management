...
| Method | Path | Purpose |
|--------|------|---------|
| `POST` | `/api/apihub/ingestion-jobs/:jobId/apply` | Marks apply outcome for a **succeeded** run (409 conflict codes for not succeeded / already applied / blocked). **P3 downstream:** optional JSON `{ "target": "sales_order" \| "purchase_order" \| "control_tower_audit", "rows"?: [...], "matchKey"?: "none" \| "sales_order_external_ref" \| "purchase_order_buyer_reference" }` applies the same mapped-row shapes as staging batch apply (requires **`org.orders` → edit** for SO/PO and **`org.controltower` → edit** for CT audit). Rows default from `resultSummary` JSON when the body omits `rows` but the summary contains a **`rows`** or **`applyRows`** array of `{ "rowIndex"?, "mappedRecord" }`. Live apply **claims** `appliedAt` in-DB first, then writes downstream in the same transaction (rolls back both on failure). **409** `APPLY_DOWNSTREAM_FAILED` when validation or row apply fails. **`matchKey`:** `sales_order_external_ref` / `purchase_order_buyer_reference` reject when the respective ref is set and a row already exists for the tenant. Success body includes **`targetSummary`** and optional **`downstreamSummary`** (per-row results). Marker-only apply (no `target`) still only sets `appliedAt`; **`targetSummary`** then comes from `resultSummary` counts when present. Dry-run **`writeSummary`** may include **`downstreamPreview`** when `target` is set. **`dryRun`:** query `?dryRun=1` (or JSON `{ "dryRun": true }`). **`idempotency-key`** (max 128 chars): stored with a **`requestFingerprint`** (marker vs downstream shape); replay requires same run id, **`dryRun`**, and fingerprint — **409** `APPLY_IDEMPOTENCY_PAYLOAD_MISMATCH` if the apply shape changed. |
