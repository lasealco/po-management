...
|--------|----------------|------|
| Purchase orders, order-linked reads | `getPurchaseOrderScopeWhere`, `purchaseOrderWhereWithViewerScope` in `src/lib/org-scope.ts` | Also re-exported from `src/lib/viewer-scopes.ts` |
