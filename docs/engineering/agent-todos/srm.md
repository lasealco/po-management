...
- [ ] **Phase K** — **Field-level permissions**, **wireframe parity**, **rules engine** — **priority 3** · roadmap: [`SRM_ROADMAP_G_I_K` § K](../../srm/SRM_ROADMAP_G_I_K.md) · **K-v1 shipped:** expanded **sensitive** redaction (PII, addresses, commercial, contacts, office locations, capability geo/notes) + UI callouts; list in `redact-supplier-sensitive.ts` · *still open: per-role field map, wireframe matrix, rules engine*
