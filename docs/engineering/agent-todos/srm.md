...
- [ ] **Phase G** — Deeper **lifecycle** (staged capture, notifications) — **priority 1** · roadmap: [`SRM_ROADMAP_G_I_K` § G](../../srm/SRM_ROADMAP_G_I_K.md) · **G-v1 shipped:** auto-**cleared** when all tasks done, notification unread filter + mark all read · **opt-in email:** `SRM_OPERATOR_EMAIL_NOTIFICATIONS=1` + Resend (`srm-operator-notification-email-mirror.ts`) · *still open: webhooks, richer automation*
