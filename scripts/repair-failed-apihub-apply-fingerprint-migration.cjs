/**
 * Fix Prisma P3018 for migration `20260430160000_apihub_apply_idempotency_fingerprint`
 * when deploy failed because requestFingerprint already exists (e.g. db push / drift).
 *
 * Uses the same URL order as prisma.config.ts (UNPOOLED / DIRECT / DATABASE).
 *
 *   npm run db:repair:apihub-apply-fingerprint-migration
 *   npm run db:repair:apihub-apply-fingerprint-migration -- --dry-run
 *
...
const MIGRATION = "20260430160000_apihub_apply_idempotency_fingerprint";
...
async function main() {
  ...
  const hasFingerprint = await columnExists(
    client,
  ...
  console.log("Database check:", {
    requestFingerprint: hasFingerprint,
  ...
  if (hasFingerprint) {
    console.log(
      "\n→ requestFingerprint column present. Marking migration applied (prisma migrate resolve --applied).\n",
    );
    if (dry) return;
  ...
  console.log(
    "\n→ Column missing. Marking failed migration rolled back so deploy can retry.\n" +
